...
> ????"??? 64 ????????"????????? 4 ????? (A):?? com_method ??????,? notepad-x64 ? Win32 WndProc ? GDI ???????????????? 4 ??? 32 ????

### 32 ???(?? 4 ??,?????)
- ?? 4 ???? COM ???(installStartupHandlers ?)**? x86/x64 ????** CoCreateInstance ??,???? CLSID ?? REGDB_E_CLASSNOTREG,??? 32 ? notepad(??? handler ????)? 32 ? 
otepad.exe ??? memory access out of bounds @ 0x41379a?
- ??:???? COM ?? if (pe.is64) { ... } ??,32 ???????????git diff -w HEAD ???????????????(??????)?

### 64 ???:4 ????????
1. **com_method ??? kernel32.dll ?? ole32.dll**:llocDynamicStub ????? hook ???????? kernel32?? hook ?? ole32.dll!com_method ? ????,????? ec=120,????????:?? kernel32.dll!com_*(?? CoCreateInstance ? pi-ms-win-core-com-l1-1-0.dll ??)?
2. **RegisterClassExW ????? x86 ??**:lpszClassName(+40)/lpszMenuName(+36) ? x86 ??;x64 WNDCLASSEXW ? 8 ????,???? **+64/+56**?lpfnWndProc(+8) ?????????? ? ????? ? createWindow ???? WndProc ? ? enqueue ????(????)???:? 	his.mode ??????
3. **DispatchMessageW ? x64 ?? return 0**:? x86 ??? 32 ?????? Executor;x64 ? if (mode==='x64') return 0 ????? x64 ??:? cx/rdx/r8/r9 ?? + 32 ?? shadow + **8 ?? sentinel ????**(rameR % 16 == 8,WndProc et ?? sentinel ? rip ? SEH sentinel ????);snapshotRegs/restoreRegs ????????? 64 ???????
4. **?????**:com_method ? self === comObj ? wait runGuiPump()????????? enqueue ? WM_CREATE/WM_PAINT;???????? WM_QUIT,??????? onMessageWait(? GetMessageW ??),??? postMessage ?? WM_PAINT/WM_QUIT?

### ??(???????)
- scripts/probe-render.ts(interactive + ?? GDI ?)? 
otepad-x64.exe:
  - [probe] bridge attached hwnd=0x10001
  - [gdi] hwnd=0x10001 createDC name=DISPLAY
  - [gdi] hwnd=0x10001 flush dc=1  ? **x64 WndProc ???????**
  - DispatchMessageW hwnd=0x10001 wndProc=0x100e950 mode=x64 ??,DefWindowProcW ???,
ested sentinel hit ? WndProc returned ?????
- un:exe notepad.exe ? exit code 0(32 ??????)?
- un:exe notepad-x64.exe ??? ? entry returned WITHOUT ExitProcess(??:???????,?? WM_CREATE/WM_PAINT ???)?
- 	sc --noEmit:??????(codegen Operand.size �6?x86-decoder:815?BrowserApp:139),**???**;itest run:259 passed / 1 ?? fail(process-manager ????,??)?

### ?????
- notepad-x64 ???(interactive + gdiBridge)???**????/??/???**;???????? XAML TextBox,???(???? WinAppSDK,????)?
- ???????? 
otepad-x64.exe ??????;?????? probe ?????
