/**
 * Probe: dump runtime BITMAPINFO + pixel rows at every SetDIBitsToDevice call.
 * Answers the row-stride question (biWidth vs dwWidth) for winmine's tiles.
 */
...
class ProbeInterceptor extends ApiInterceptorImpl implements ApiInterceptor {
  ...
  override async dispatch(ctx: ApiCallContext): Promise<ApiResult> {
    if (/SetDIBitsToDevice/i.test(ctx.proc) && this.n < 4) {
      this.n++;
      ...
      sync(`[sdib#${this.n}] hdc=0x${(hdc ?? 0).toString(16)} dest=(${xd},${yd}) srcSize=${dw}x${dh} startScan=${us} cScan=${cs} bits=0x${(bits ?? 0).toString(16)} bmi=0x${(bmi ?? 0).toString(16)}`);
      sync(`[sdib#${this.n}] BITMAPINFO: size=${biSize} w=${biWidth} h=${biHeight} bitCount=${biBitCount} comp=${biCompression} clrUsed=${biClrUsed}`);
      // Dump the first 8 rows of pixel data as nibbles, using biWidth stride.
      if (biWidth > 0 && biBitCount === 4 && bits) {
        ...
        for (let r = 0; r < 8; r++) {
          ...
          for (let i = 0; i < stride; i++) {
            ...
            pix.push(`${(b >> 4).toString(16)}${(b & 0xf).toString(16)}`);
          }
          sync(`[sdib#${this.n}] row${r} stride=${stride}: ${pix.join(' ')}`);
        }
      }
    }
